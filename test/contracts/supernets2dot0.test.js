...
describe('Polygon ZK-EVM', () => {
    ...
    it('should check the constructor parameters', async () => {
        expect(await supernets2dot0Contract.globalExitRootManager()).to.be.equal(supernets2dot0GlobalExitRoot.address);
        expect(await supernets2dot0Contract.matic()).to.be.equal(maticTokenContract.address);
        expect(await supernets2dot0Contract.rollupVerifier()).to.be.equal(verifierContract.address);
        expect(await supernets2dot0Contract.bridgeAddress()).to.be.equal(supernets2dot0BridgeContract.address);

        expect(await supernets2dot0Contract.owner()).to.be.equal(deployer.address);
        expect(await supernets2dot0Contract.admin()).to.be.equal(admin.address);
        expect(await supernets2dot0Contract.chainID()).to.be.equal(chainID);
        expect(await supernets2dot0Contract.trustedSequencer()).to.be.equal(trustedSequencer.address);
        expect(await supernets2dot0Contract.pendingStateTimeout()).to.be.equal(pendingStateTimeoutDefault);
        expect(await supernets2dot0Contract.trustedAggregator()).to.be.equal(trustedAggregator.address);
        expect(await supernets2dot0Contract.trustedAggregatorTimeout()).to.be.equal(trustedAggregatorTimeoutDefault);

        expect(await supernets2dot0Contract.batchNumToStateRoot(0)).to.be.equal(genesisRoot);
        expect(await supernets2dot0Contract.trustedSequencerURL()).to.be.equal(urlSequencer);
        expect(await supernets2dot0Contract.networkName()).to.be.equal(networkName);

        expect(await supernets2dot0Contract.batchFee()).to.be.equal(ethers.utils.parseEther('0.1'));
        expect(await supernets2dot0Contract.batchFee()).to.be.equal(ethers.utils.parseEther('0.1'));
        expect(await supernets2dot0Contract.getForcedBatchFee()).to.be.equal(ethers.utils.parseEther('10'));

        expect(await supernets2dot0Contract.forceBatchTimeout()).to.be.equal(FORCE_BATCH_TIMEOUT);
        expect(await supernets2dot0Contract.isForcedBatchDisallowed()).to.be.equal(true);
    });

    it('should check initialize function', async () => {
        const Supernets2dot0Factory = await ethers.getContractFactory('Supernets2dot0Mock');
        const supernets2dot0ContractInitialize = await upgrades.deployProxy(Supernets2dot0Factory, [], {
            initializer: false,
            constructorArgs: [
                supernets2dot0GlobalExitRoot.address,
                maticTokenContract.address,
        ...
        await expect(supernets2dot0ContractInitialize.initialize(
            {
                admin: admin.address,
                trustedSequencer: trustedSequencer.address,
        ...
        await expect(
            supernets2dot0ContractInitialize.initialize(
                {
                    admin: admin.address,
                    trustedSequencer: trustedSequencer.address,
                    pendingStateTimeout: pendingStateTimeoutDefault,
        ...
        ).to.emit(supernets2dot0ContractInitialize, 'UpdateZkEVMVersion').withArgs(0, forkID, version);
    });
    ...
    it('should check setters of admin', async () => {
        ...
        // setTrustedSequencer
        await expect(supernets2dot0Contract.setTrustedSequencer(deployer.address))
            .to.be.revertedWith('OnlyAdmin');
        await expect(
            supernets2dot0Contract.connect(admin).setTrustedSequencer(deployer.address),
        ).to.emit(supernets2dot0Contract, 'SetTrustedSequencer').withArgs(deployer.address);
        expect(await supernets2dot0Contract.trustedSequencer()).to.be.equal(deployer.address);

        // setTrustedSequencerURL
        const url = 'https://test';
        await expect(supernets2dot0Contract.setTrustedSequencerURL(url))
            .to.be.revertedWith('OnlyAdmin');
        await expect(
            supernets2dot0Contract.connect(admin).setTrustedSequencerURL(url),
        ).to.emit(supernets2dot0Contract, 'SetTrustedSequencerURL').withArgs(url);
        expect(await supernets2dot0Contract.trustedSequencerURL()).to.be.equal(url);

        // setTrustedAggregator
        const newTrustedAggregator = deployer.address;
        await expect(supernets2dot0Contract.setTrustedAggregator(newTrustedAggregator))
            .to.be.revertedWith('OnlyAdmin');
        await expect(
            supernets2dot0Contract.connect(admin).setTrustedAggregator(newTrustedAggregator),
        ).to.emit(supernets2dot0Contract, 'SetTrustedAggregator').withArgs(newTrustedAggregator);
        expect(await supernets2dot0Contract.trustedAggregator()).to.be.equal(newTrustedAggregator);

        // setTrustedAggregatorTimeout
        await expect(supernets2dot0Contract.setTrustedAggregatorTimeout(trustedAggregatorTimeoutDefault))
            .to.be.revertedWith('OnlyAdmin');

        await expect(supernets2dot0Contract.connect(admin).setTrustedAggregatorTimeout(HALT_AGGREGATION_TIMEOUT + 1))
            .to.be.revertedWith('TrustedAggregatorTimeoutExceedHaltAggregationTimeout');

        await expect(supernets2dot0Contract.connect(admin).setTrustedAggregatorTimeout(trustedAggregatorTimeoutDefault))
        ...
        await expect(
            supernets2dot0Contract.connect(admin).setTrustedAggregatorTimeout(newTrustedAggregatorTimeout),
        ).to.emit(supernets2dot0Contract, 'SetTrustedAggregatorTimeout').withArgs(newTrustedAggregatorTimeout);
        expect(await supernets2dot0Contract.trustedAggregatorTimeout()).to.be.equal(newTrustedAggregatorTimeout);

        // setPendingStateTimeoutDefault
        await expect(supernets2dot0Contract.setPendingStateTimeout(pendingStateTimeoutDefault))
            .to.be.revertedWith('OnlyAdmin');

        await expect(supernets2dot0Contract.connect(admin).setPendingStateTimeout(HALT_AGGREGATION_TIMEOUT + 1))
            .to.be.revertedWith('PendingStateTimeoutExceedHaltAggregationTimeout');

        await expect(supernets2dot0Contract.connect(admin).setPendingStateTimeout(pendingStateTimeoutDefault))
            .to.be.revertedWith('NewPendingStateTimeoutMustBeLower');

        const newPendingStateTimeoutDefault = pendingStateTimeoutDefault - 1;
        await expect(
            supernets2dot0Contract.connect(admin).setPendingStateTimeout(newPendingStateTimeoutDefault),
        ).to.emit(supernets2dot0Contract, 'SetPendingStateTimeout').withArgs(newPendingStateTimeoutDefault);
        expect(await supernets2dot0Contract.pendingStateTimeout()).to.be.equal(newPendingStateTimeoutDefault);

        // setMultiplierBatchFee
        const newMultiplierBatchFee = 1023;
        await expect(supernets2dot0Contract.connect(admin).setMultiplierBatchFee(newMultiplierBatchFee + 1))
            .to.be.revertedWith('InvalidRangeMultiplierBatchFee');

        await expect(
            supernets2dot0Contract.connect(admin).setMultiplierBatchFee(newMultiplierBatchFee),
        ).to.emit(supernets2dot0Contract, 'SetMultiplierBatchFee').withArgs(newMultiplierBatchFee);
        expect(await supernets2dot0Contract.multiplierBatchFee()).to.be.equal(newMultiplierBatchFee);

        // setVerifyBatchTimeTarget
        const newVerifyBatchTimeTarget = 100;

        await expect(supernets2dot0Contract.connect(admin).setVerifyBatchTimeTarget(60 * 60 * 24 + 1)) // more than 1 day
            .to.be.revertedWith('InvalidRangeBatchTimeTarget');

        await expect(
            supernets2dot0Contract.connect(admin).setVerifyBatchTimeTarget(newVerifyBatchTimeTarget),
        ).to.emit(supernets2dot0Contract, 'SetVerifyBatchTimeTarget').withArgs(newVerifyBatchTimeTarget);
        expect(await supernets2dot0Contract.verifyBatchTimeTarget()).to.be.equal(newVerifyBatchTimeTarget);

        // setPendingStateTimeoutDefault
        const newForceBatchTimeout = 0;
        await expect(supernets2dot0Contract.setForceBatchTimeout(newForceBatchTimeout))
            .to.be.revertedWith('OnlyAdmin');

        await expect(supernets2dot0Contract.connect(admin).setForceBatchTimeout(HALT_AGGREGATION_TIMEOUT + 1))
            .to.be.revertedWith('InvalidRangeForceBatchTimeout');

        await expect(supernets2dot0Contract.connect(admin).setForceBatchTimeout(FORCE_BATCH_TIMEOUT))
            .to.be.revertedWith('InvalidRangeForceBatchTimeout');
        await expect(
            supernets2dot0Contract.connect(admin).setForceBatchTimeout(newForceBatchTimeout),
        ).to.emit(supernets2dot0Contract, 'SetForceBatchTimeout').withArgs(newForceBatchTimeout);
        expect(await supernets2dot0Contract.forceBatchTimeout()).to.be.equal(newForceBatchTimeout);

        // Activate force batches
        await expect(supernets2dot0Contract.activateForceBatches())
            .to.be.revertedWith('OnlyAdmin');

        // Check force batches are unactive
        await expect(supernets2dot0Contract.forceBatch('0x', 0))
            .to.be.revertedWith('ForceBatchNotAllowed');
        await expect(supernets2dot0Contract.sequenceForceBatches([]))
            .to.be.revertedWith('ForceBatchNotAllowed');

        await expect(
            supernets2dot0Contract.connect(admin).activateForceBatches(),
        ).to.emit(supernets2dot0Contract, 'ActivateForceBatches');
        await expect(supernets2dot0Contract.connect(admin).activateForceBatches())
            .to.be.revertedWith('ForceBatchesAlreadyActive');

        expect(await supernets2dot0Contract.isForcedBatchDisallowed()).to.be.equal(false);

        // Transfer admin role

        // First set pending Admin
        expect(await supernets2dot0Contract.pendingAdmin()).to.be.equal(ethers.constants.AddressZero);
        await expect(supernets2dot0Contract.transferAdminRole(deployer.address))
            .to.be.revertedWith('OnlyAdmin');

        await expect(
            supernets2dot0Contract.connect(admin).transferAdminRole(deployer.address),
        ).to.emit(supernets2dot0Contract, 'TransferAdminRole').withArgs(deployer.address);
        expect(await supernets2dot0Contract.pendingAdmin()).to.be.equal(deployer.address);

        // Accept transfer admin
        expect(await supernets2dot0Contract.admin()).to.be.equal(admin.address);
        await expect(supernets2dot0Contract.connect(admin).acceptAdminRole())
            .to.be.revertedWith('OnlyPendingAdmin');

        await expect(
            supernets2dot0Contract.connect(deployer).acceptAdminRole(),
        ).to.emit(supernets2dot0Contract, 'AcceptAdminRole').withArgs(deployer.address);
        expect(await supernets2dot0Contract.admin()).to.be.equal(deployer.address);
    });
    ...
    it('should check state roots inside prime', async () => {
        ...
        for (let i = 0; i < validRoots.length; i++) {
            expect(await supernets2dot0Contract.checkStateRootInsidePrime(validRoots[i])).to.be.equal(true);
        }

        for (let i = 0; i < aliasInvalidRoots.length; i++) {
            expect(await supernets2dot0Contract.checkStateRootInsidePrime(aliasInvalidRoots[i])).to.be.equal(false);
        }
    });
    ...
    it('should sequence a batch as trusted sequencer', async () => {
        ...
        await expect(supernets2dot0Contract.sequenceBatches([sequence], trustedSequencer.address))
        ...
        await expect(supernets2dot0Contract.connect(trustedSequencer).sequenceBatches([], trustedSequencer.address))
        ...
        // Sequence batch
        await expect(supernets2dot0Contract.connect(trustedSequencer).sequenceBatches([sequence], deployer.address))
        ...
        // Check batch mapping
        const sequencedBatchData = await supernets2dot0Contract.sequencedBatches(1);
        const batchAccInputHash = sequencedBatchData.accInputHash;

        const batchAccInputHashJs = calculateAccInputHash(
            (await supernets2dot0Contract.sequencedBatches(0)).accInputHash,
    ...
    it('sequenceBatches should sequence multiple batches', async () => {
        ...
        const maticAmount = (await supernets2dot0Contract.batchFee()).mul(2);
        ...
        // Sequence batches
        await expect(supernets2dot0Contract.connect(trustedSequencer).sequenceBatches([sequence, sequence2], trustedSequencer.address))
            .to.emit(supernets2dot0Contract, 'SequenceBatches')
        ...
        const sequencedBatchData2 = await supernets2dot0Contract.sequencedBatches(2);
    ...
    it('force batches through smart contract', async () => {
        ...
        const maticAmount = await supernets2dot0Contract.getForcedBatchFee();
        const lastGlobalExitRoot = await supernets2dot0GlobalExitRoot.getLastGlobalExitRoot();
        ...
        // Approve matic
        const approveTx = await maticTokenContract.populateTransaction.approve(supernets2dot0Contract.address, maticAmount);
        ...
        // Force batch
        const lastForcedBatch = (await supernets2dot0Contract.lastForceBatch()) + 1;

        const forceBatchTx = await supernets2dot0Contract.populateTransaction.forceBatch(l2txDataForceBatch, maticAmount);
        await expect(sendDataContract.sendData(forceBatchTx.to, forceBatchTx.data))
            .to.emit(supernets2dot0Contract, 'ForceBatch')
    ...
    it('sequenceBatches should sequence multiple batches and force batches', async () => {
        ...
        await expect(
            maticTokenContract.approve(supernets2dot0Contract.address, maticAmount),
        ).to.emit(maticTokenContract, 'Approval');
        ...
        // Force batch
        await expect(supernets2dot0Contract.forceBatch(l2txDataForceBatch, maticAmount))
            .to.emit(supernets2dot0Contract, 'ForceBatch')
            .withArgs(lastForcedBatch, lastGlobalExitRoot, deployer.address, '0x');

        // sequence 2 batches
        const l2txData = '0x1234';
        const maticAmountSequence = (await supernets2dot0Contract.batchFee()).mul(1);
        ...
        // Approve tokens
        await expect(
            maticTokenContract.connect(trustedSequencer).approve(supernets2dot0Contract.address, maticAmountSequence),
        ).to.emit(maticTokenContract, 'Approval');
        ...
        // Check batch mapping
        const batchAccInputHash = (await supernets2dot0Contract.sequencedBatches(1)).accInputHash;
        // Only last batch is added to the mapping
        expect(batchAccInputHash).to.be.equal(ethers.constants.HashZero);
        ...
        const batchData2 = await supernets2dot0Contract.sequencedBatches(2);
    ...
    it('should force a batch of transactions', async () => {
        ...
        expect(maticAmount.toString()).to.be.equal((await supernets2dot0Contract.getForcedBatchFee()).toString());
        ...
        await expect(supernets2dot0Contract.forceBatch(l2txData, maticAmount.sub(1)))
        ...
        const lastForceBatch = await supernets2dot0Contract.lastForceBatch();
        ...
        const batchHash = await supernets2dot0Contract.forcedBatches(1);
    ...
    it('should sequence force batches using sequenceForceBatches', async () => {
        ...
        const forceBatchHash = await supernets2dot0Contract.forcedBatches(1);
        ...
        expect(await supernets2dot0Contract.lastForceBatchSequenced()).to.be.equal(0);
        expect(await supernets2dot0Contract.lastForceBatch()).to.be.equal(1);
        expect(await supernets2dot0Contract.lastBatchSequenced()).to.be.equal(0);
        ...
        await expect(supernets2dot0Contract.sequenceForceBatches(Array(2).fill(forceBatchStruct)))
            .to.be.revertedWith('ForceBatchesOverflow');

        // revert because the timeout is not expired
        await expect(supernets2dot0Contract.sequenceForceBatches([forceBatchStruct]))
        ...
        await expect(supernets2dot0Contract.sequenceForceBatches([forceBatchStructBad]))
            .to.be.revertedWith('ForcedDataDoesNotMatch');
        forceBatchStructBad.minForcedTimestamp -= 1;
        ...
        await expect(supernets2dot0Contract.sequenceForceBatches([forceBatchStruct]))
            .to.emit(supernets2dot0Contract, 'SequenceForceBatches')
            .withArgs(1);

        const timestampSequenceBatch = (await ethers.provider.getBlock()).timestamp;

        expect(await supernets2dot0Contract.lastForceBatchSequenced()).to.be.equal(1);
        expect(await supernets2dot0Contract.lastForceBatch()).to.be.equal(1);
        expect(await supernets2dot0Contract.lastBatchSequenced()).to.be.equal(1);
    ...
    it('should verify a sequenced batch using verifyBatchesTrustedAggregator', async () => {
        ...
        const numBatch = (await supernets2dot0Contract.lastVerifiedBatch()) + 1;
        ...
        await expect(
            supernets2dot0Contract.connect(deployer).verifyBatchesTrustedAggregator(
                pendingState,
                numBatch - 1,
                numBatch - 1,
                newLocalExitRoot,
        ...
        await expect(
            supernets2dot0Contract.connect(trustedAggregator).verifyBatchesTrustedAggregator(
                pendingState,
                numBatch - 1,
                numBatch - 1,
                newLocalExitRoot,
    ...
    it('should verify forced sequenced batch using verifyBatchesTrustedAggregator', async () => {
        ...
        ).to.emit(supernets2dot0Contract, 'VerifyBatch')
            .withArgs(numBatch, trustedAggregator.address)
            .to.emit(maticTokenContract, 'Transfer')
            .withArgs(supernets2dot0Contract.address, trustedAggregator.address, maticAmount);
    ...
    it('should match the computed SC input with the Js input', async () => {
        ...
        const sentBatchHash = (await supernets2dot0Contract.sequencedBatches(lastBatchSequenced + 1)).accInputHash;
        const oldAccInputHash = (await supernets2dot0Contract.sequencedBatches(0)).accInputHash;
        ...
        const currentStateRoot = await supernets2dot0Contract.batchNumToStateRoot(0);
        ...
        const circuitInpuSnarkSC = await supernets2dot0Contract.getNextSnarkInput(
            pendingStateNum,
            numBatch - 1,
            numBatch,
            newLocalExitRoot,
    ...
    it('should match the computed SC input with the Js input in force batches', async () => {
        ...
        const lastForcedBatch = (await supernets2dot0Contract.lastForceBatch()).toNumber() + 1;
        await expect(supernets2dot0Contract.forceBatch(l2txData, maticAmount))
            .to.emit(supernets2dot0Contract, 'ForceBatch')
            .withArgs(lastForcedBatch, lastGlobalExitRoot, deployer.address, '0x');
    ...
    it('should verify a sequenced batch using verifyBatches', async () => {
        ...
        const currentBatchFee = await supernets2dot0Contract.batchFee();

        await expect(
            supernets2dot0Contract.connect(aggregator1).verifyBatches(
                pendingState,
                numBatch - 1,
                numBatch,
                newLocalExitRoot,
        ...
        expect(lastPendingstate).to.be.equal(await supernets2dot0Contract.lastPendingState());

        const pendingStateData = await supernets2dot0Contract.pendingStateTransitions(lastPendingstate);
        ...
        expect(0).to.be.equal(await supernets2dot0Contract.lastVerifiedBatch());

        // Pending state can't be 0
        await expect(
            supernets2dot0Contract.consolidatePendingState(0),
        ).to.be.revertedWith('PendingStateInvalid');

        // Pending state does not exist
        await expect(
            supernets2dot0Contract.consolidatePendingState(2),
        ).to.be.revertedWith('PendingStateInvalid');

        // Not ready to be consolidated
        await expect(
            supernets2dot0Contract.consolidatePendingState(lastPendingstate),
        ...
        await expect(
            supernets2dot0Contract.consolidatePendingState(lastPendingstate),
        ).to.emit(supernets2dot0Contract, 'ConsolidatePendingState')
            .withArgs(numBatch, newStateRoot, lastPendingstate);

        // Pending state already consolidated
        await expect(
            supernets2dot0Contract.consolidatePendingState(1),
        ).to.be.revertedWith('PendingStateInvalid');

        // Fee es divided because is was fast verified
        const multiplierFee = await supernets2dot0Contract.multiplierBatchFee();
        expect((currentBatchFee.mul(1000)).div(multiplierFee)).to.be.equal(await supernets2dot0Contract.batchFee());

        // Check pending state variables
        expect(1).to.be.equal(await supernets2dot0Contract.lastVerifiedBatch());
        expect(newStateRoot).to.be.equal(await supernets2dot0Contract.batchNumToStateRoot(1));
        expect(1).to.be.equal(await supernets2dot0Contract.lastPendingStateConsolidated());
    });
    ...
    it('should test the pending state properly', async () => {
        ...
        await expect(
            maticTokenContract.connect(trustedSequencer).approve(supernets2dot0Contract.address, maticTokenInitialBalance),
        ).to.emit(maticTokenContract, 'Approval');

        // Make 20 sequences of 5 batches, with 1 minut timestamp difference
        for (let i = 0; i < 20; i++) {
            await expect(supernets2dot0Contract.connect(trustedSequencer).sequenceBatches(sequencesArray, trustedSequencer.address))
                .to.emit(supernets2dot0Contract, 'SequenceBatches');
        }
        ...
        // Check pending state
        currentPendingState++;
        expect(currentPendingState).to.be.equal(await supernets2dot0Contract.lastPendingState());

        let currentPendingStateData = await supernets2dot0Contract.pendingStateTransitions(currentPendingState);
        ...
        expect(currentPendingState).to.be.equal(await supernets2dot0Contract.lastPendingState());
        expect(0).to.be.equal(await supernets2dot0Contract.lastPendingStateConsolidated());

        // Check consolidated state
        let currentVerifiedBatch = newBatch;
        expect(currentVerifiedBatch).to.be.equal(await supernets2dot0Contract.lastVerifiedBatch());
        expect(newStateRoot).to.be.equal(await supernets2dot0Contract.batchNumToStateRoot(currentVerifiedBatch));
        ...
        expect(ethers.constants.HashZero).to.be.equal(await supernets2dot0Contract.batchNumToStateRoot(currentNumBatch));
        await expect(
            supernets2dot0Contract.connect(trustedAggregator).verifyBatchesTrustedAggregator(
                currentPendingState,
                currentNumBatch,
        ...
        currentPendingStateData = await supernets2dot0Contract.pendingStateTransitions(currentPendingState);
        expect(verifyTimestamp).to.be.equal(currentPendingStateData.timestamp);
        expect(newBatch).to.be.equal(currentPendingStateData.lastVerifiedBatch);
        expect(newLocalExitRoot).to.be.equal(currentPendingStateData.exitRoot);
        expect(newStateRoot).to.be.equal(currentPendingStateData.stateRoot);
        ...
        const firstPendingState = await supernets2dot0Contract.pendingStateTransitions(1);
        ...
        currentPendingState++;
        expect(currentPendingState).to.be.equal(await supernets2dot0Contract.lastPendingState());
        expect(currentPendingConsolidated).to.be.equal(await supernets2dot0Contract.lastPendingStateConsolidated());
        ...
        const secondPendingState = await supernets2dot0Contract.pendingStateTransitions(2);
        ...
        currentPendingConsolidated = await supernets2dot0Contract.lastPendingStateConsolidated();
        const lastPendingState = await supernets2dot0Contract.pendingStateTransitions(currentPendingState);
        ...
        const nextConsolidatedState = await supernets2dot0Contract.pendingStateTransitions(nextConsolidatedStateNum);
        ...
        await expect(
            supernets2dot0Contract.connect(admin).setPendingStateTimeout(0),
        ).to.emit(supernets2dot0Contract, 'SetPendingStateTimeout').withArgs(0);
    ...
    it('Activate emergency state due halt timeout', async () => {
        ...
        const sequencedTimestmap = Number((await supernets2dot0Contract.sequencedBatches(1)).sequencedTimestamp);
        const haltTimeout = HALT_AGGREGATION_TIMEOUT;

        // Try to activate the emergency state

        // Check batch is not sequenced
        await expect(supernets2dot0Contract.connect(aggregator1).activateEmergencyState(2))
            .to.be.revertedWith('BatchNotSequencedOrNotSequenceEnd');

        // Check batch is already verified
        await supernets2dot0Contract.setVerifiedBatch(1);
        await expect(supernets2dot0Contract.connect(aggregator1).activateEmergencyState(1))
            .to.be.revertedWith('BatchAlreadyVerified');
        await supernets2dot0Contract.setVerifiedBatch(0);
    ...
    it('Test overridePendingState properly', async () => {
        ...
        await expect(
            supernets2dot0Contract.connect(aggregator1).overridePendingState(
                currentPendingState,
                finalPendingState,
                currentNumBatch,
                newBatch,
        ...
        await expect(
            supernets2dot0Contract.connect(trustedAggregator).overridePendingState(
                finalPendingState + 1,
                finalPendingState + 2,
                currentNumBatch,
                newBatch,
        ...
        ).to.emit(supernets2dot0Contract, 'OverridePendingState').withArgs(newBatch, newStateRoot2, trustedAggregator.address);
        ...
        expect(currentVerifiedBatch).to.be.equal(await supernets2dot0Contract.lastVerifiedBatch());
        expect(newStateRoot2).to.be.equal(await supernets2dot0Contract.batchNumToStateRoot(currentVerifiedBatch));
    });

    it('Test batch fees properly', async () => {
        const accInputData = ethers.constants.HashZero;
        const verifyBatchTimeTarget = Number(await supernets2dot0Contract.verifyBatchTimeTarget());
        const currentTimestamp = (await ethers.provider.getBlock()).timestamp;

        const multiplierFee = ethers.BigNumber.from(await supernets2dot0Contract.multiplierBatchFee()); // 1002
        const bingNumber1000 = ethers.BigNumber.from(1000);

        // Create sequenced to update the fee
        await supernets2dot0Contract.setSequencedBatches(
            50,
            accInputData,
        ...
        // Assert currentFee
        let currentBatchFee = await supernets2dot0Contract.batchFee();
        ...
        await supernets2dot0Contract.updateBatchFee(100);

        // Fee does not change since there are the same batches above than below
        expect(await supernets2dot0Contract.batchFee()).to.be.equal(currentBatchFee);
        ...
        expect(currentBatchFee).to.be.equal(await supernets2dot0Contract.batchFee());

        // Check the fee is now below
        await supernets2dot0Contract.setSequencedBatches(50, accInputData, currentTimestamp + verifyBatchTimeTarget * 2, 0); // Below

 ...
 */
contract Supernets2dot0Timelock is TimelockController {
    // Polygon ZK-EVM address. Will be used to check if it's on emergency state.
    Supernets2dot0 public immutable supernets2dot0;
     ...
     * @param _supernets2dot0 supernets2dot0 address
     **/
    constructor(
        uint256 minDelay,
        ...
        address admin,
        Supernets2dot0 _supernets2dot0
    ...
    function getMinDelay() public view override returns (uint256 duration) {
        if (address(supernets2dot0) != address(0) && supernets2dot0.isEmergencyState()) {
            return 0;
        } else {
